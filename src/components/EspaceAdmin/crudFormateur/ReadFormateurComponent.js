...
import { Table, Button, Modal, Typography, Space, Input, Form, Select, Upload, Popconfirm, message } from "antd";
...
const ReadFormateurComponent = () => {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            cancelText="Non"
          >
            <Button type="dashed" icon={<DeleteOutlined style={{ color: "darkred" }} />} />
          </Popconfirm>
        </Space>
      ),
      width: 200,
    ...
    form.setFieldsValue({
      ...
      cv_pdf: record.cv_pdf
        ? [
            {
              uid: '-1',
              name: record.cv_pdf.split('/').pop(),
              status: 'done',
              url: `http://localhost:8001/${record.cv_pdf}`,
            },
          ]
        : [],
      photo: record.photo
        ? [
            {
              ...
              status: 'done',
              url: `http://localhost:8001/${record.photo}`,
            },
          ]
        : [],
    });
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      formData.append("role", "formateur"); // Spécifier le rôle
      ...
      // Ajouter les mots de passe uniquement pour l'ajout ou si modifiés
      if (!isEditMode || values.mot_de_passe) {
      ...
      if (isEditMode) {
        ...
        message.success("Formateur modifié avec succès");
      } else {
        ...
        // Mettre à jour l'UI immédiatement avec le nouveau formateur
        if (response.data && response.data.user) {
          setFormateurs(prev => [...prev, { ...response.data.user, key: response.data.user._id }]);
        }
      }

      // Recharger les formateurs pour être sûr d'avoir les données à jour
  ...
  return (
    <div style={{ margin: "20px", backgroundColor: "#fff", padding: "20px", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0, 0, 0, 0.1)" }}>
      ...
      />
      <Modal
        title={isEditMode ? "Modifier le freelancer" : isViewMode ? "Détails du freelancer" : "Ajouter un freelancer"}
        ...
        {isViewMode ? (
          <div style={{
            ...
            <div style={{ width: "100%", maxWidth: "800px" }}>
              ...
              <div style={{
                ...
                padding: "20px",
                borderRadius: "8px",
                boxShadow: "0 2 px 4px rgba(0, 0, 0, 0.1)"
              }}>
                {[
                  ...
                  { label: "CV", value: modalContent.cv_pdf ? <a href={`http://localhost:8001/${modalContent.cv_pdf}`} target="_blank" rel="noopener noreferrer"><EyeOutlined /> Voir CV</a> : 'Aucun CV' },
                  { label: "Spécialité", value: modalContent.specialite || 'Aucune spécialité' },
                ].map((item, index) => (
          ...
          >
            <Form.Item 
              label="Nom" 
              name="nom" 
              rules={[{ required: true, message: "Veuillez entrer le nom du formateur" }]}
            >
              <Input />
            </Form.Item>
            <Form.Item 
              label="Prénom" 
              name="prenom" 
              rules={[{ required: true, message: "Veuillez entrer le prénom du formateur" }]}
            >
              <Input />
            </Form.Item>
            <Form.Item 
              ...
              name="email" 
              rules={[
                { required: true, message: "Veuillez entrer l'email du formateur" },
            ...
            <Form.Item label="Spécialité" name="specialite">
              <Input />
            </Form.Item>
            <Form.Item
              label="Photo"
              ...
              rules={[{ required: !isEditMode, message: "Veuillez télécharger une photo" }]}
            >
              <Upload
                listType="picture"
                accept="image/*"
            ...
            <Form.Item
              label="Mot de passe"
              name="mot_de_passe"
              rules={[{ required: !isEditMode, message: "Veuillez entrer un mot de passe" }]}
            >
              <Input.Password placeholder={isEditMode ? "Laisser vide pour ne pas modifier" : ""} />
            </Form.Item>
            <Form.Item
              ...
              rules={[
                { required: !isEditMode, message: "Veuillez confirmer le mot de passe" },
                ({ getFieldValue }) => ({
                  validator(_, value) {
